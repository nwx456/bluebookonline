...
describe("questionMatchesBucketLabel", () => {
  it("rejects Module 1 label for M2 easy bucket", () => {
    const bucket = {
      section: "rw",
      module: 2,
      variant: "easy",
    ...
    assert.equal(
      questionMatchesBucketLabel(
        { sat_pdf_module_label: "Reading and Writing Module 1" },
        bucket
      ),
      false
    );
  ...
  it("allows unknown labels", () => {
    ...
    assert.equal(
      questionMatchesBucketLabel({ sat_pdf_module_label: "unknown" }, bucket),
      true
    );
  });
});
