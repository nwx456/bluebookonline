...
export async function handleApAnalyze(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    if (getExamProgram(subject as SubjectKey) !== "AP") {
      return NextResponse.json(
        { error: "SAT uploads must use the JSON analyze endpoint." },
        { status: 400 }
      );
    }
    ...
    questions = questions.filter((q) => {
      ...
      return [opts.option_a, opts.option_b, opts.option_c, opts.option_d, opts.option_e].some(
        (o) => o != null && o.trim() !== ""
      );
    });
    ...
    const rows = questions.slice(0, questionCount).map((q, i) => {
      ...
      if (shared.intro?.trim() && shared.stem?.trim()) {
        ...
        passageText = passageText?.trim() ? `${intro}\n\n${passageText}` : intro;
      }
      ...
      const preconditionText = (q.precondition ?? "").trim() || null;
    ...
    console.error("AP upload analyze error:", err);
    return NextResponse.json(
