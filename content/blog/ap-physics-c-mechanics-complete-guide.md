---
title: 'AP Physics C: Mechanics Complete Study Guide (2026 Exam)'
description: >-
  Master AP Physics C Mechanics for May 2026: calculus-based mechanics, 40 MCQs
  + 4 FRQs, unit weights, and study plan.
date: '2026-07-19'
updated: '2026-07-19'
author: AP Practice Exam Online Team
category: Study Guides
focusKeyword: ap physics c mechanics study guide
seoTitle: AP Physics C Mechanics Study Guide 2026
metaDescription: >-
  Complete AP Physics C Mechanics guide: calculus-based FRQs, unit weights, and
  free practice + score calculator.
tags:
  - AP Physics C
  - Study Guide
draft: false
faq:
  - question: Is Physics C harder than Physics 1?
    answer: >-
      Physics C uses calculus and is typically taken by STEM majors. Pass rates
      are higher (~72%) but the math demand is greater.
  - question: What are the four FRQ types?
    answer: >-
      Mathematical Routines, Translation Between Representations, Experimental
      Design, and Qualitative/Quantitative Translation.
  - question: Can I take Mechanics without Physics 1?
    answer: >-
      Yes, if you have strong calculus and mechanics background. Many students
      take Mechanics concurrently with or after Calc AB/BC.
---
**Short answer:** The 2026 AP Physics C: Mechanics exam is Wed, May 13, 2026 (12 PM Local) — 40 MCQs, 180 minutes total, hybrid digital (MCQ in Bluebook, paper FRQs).

The 2026 AP Physics C: Mechanics exam is **Wednesday, May 13, 2026 (12 PM local)**. Calculus-based mechanics for future engineers and physics majors.

## Exam Structure

Hybrid digital — **3 hours**. 40 MCQs (50%) + 4 FRQs (50%). Calculator on both sections.

| FRQ | Type | Max Points |
|---|---|---|
| 1 | Mathematical Routines | 10 |
| 2 | Translation Between Representations | 12 |
| 3 | Experimental Design | 10 |
| 4 | Qualitative/Quantitative Translation | 8 |

## Unit Weights

| Unit | Topic | Weight |
|---|---|---|
| 1 | Kinematics | 10–15% |
| 2 | Force and Dynamics | 20–30% |
| 3 | Work, Energy, Power | 13–20% |
| 4 | Linear Momentum | 10–18% |
| 5 | Torque and Rotational Dynamics | 10–18% |
| 6 | Rotating Systems | 10–15% |
| 7 | Oscillations | 5–10% |

Unit 2 (dynamics) is the single heaviest unit. Integrate calculus: \(a = dv/dt\), \(W = \int F \cdot dx\).

## Study Plan

Master **differential equations for motion**, **rotational kinematics with \(\alpha\)**, and **energy in non-inertial contexts**. Use released 2025 scoring guidelines from AP Central.

Track progress: [Physics C Mechanics score calculator](/tools/ap-score-calculator/ap-physics-c-mechanics) · [Practice tests](/exams/ap-physics-c-mechanics)

## Related Guides

- [Physics 1 vs 2 vs C](/blog/ap-physics-1-vs-2-vs-c)
- [How AP exams are scored](/blog/how-ap-exams-are-scored)

---

*Sources: College Board AP Physics C: Mechanics CED and 2025 Scoring Guidelines (apcentral.collegeboard.org). Verified July 2026.*
