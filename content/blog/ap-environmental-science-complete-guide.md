---
title: "AP Environmental Science Complete Study Guide (2026 Exam)"
description: "Master APES for May 2026: fully digital format, 80 MCQs + 3 FRQs, 9 units with weights, and a focused study plan."
date: "2026-07-19"
updated: "2026-07-19"
author: "AP Practice Exam Online Team"
category: "Study Guides"
focusKeyword: "ap environmental science study guide"
seoTitle: "AP Environmental Science Study Guide 2026"
metaDescription: "Complete APES guide: 80 MCQs (60%) + 3 FRQs (40%), unit weights, FRQ types, and free practice + score calculator."
tags: ["AP Environmental Science", "APES", "Study Guide"]
draft: false
faq:
  - question: "Is APES fully digital in 2026?"
    answer: "Yes. Both MCQs and typed FRQs are completed in Bluebook — no paper booklet."
  - question: "What are the three FRQ types?"
    answer: "Design an Investigation, Analyze an Environmental Problem, and Analyze with Calculation."
  - question: "Is a calculator allowed?"
    answer: "Yes — four-function, scientific, or graphing calculator on both sections."
---

The 2026 AP Environmental Science exam is **Friday, May 15, 2026 (8 AM local)**. Interdisciplinary: biology, earth science, chemistry, and policy.

## Exam Structure

Fully digital — **2 hours 40 minutes**.

| Section | Questions | Weight |
|---|---|---|
| Section I (MCQ) | 80 | 60% |
| Section II (FRQ) | 3 (10 pts each) | 40% |

## Unit Weights

| Unit | Topic | Weight |
|---|---|---|
| 1 | Ecosystems | 6–8% |
| 2 | Biodiversity | 6–8% |
| 3 | Populations | 10–15% |
| 4 | Earth Systems and Resources | 10–15% |
| 5 | Land and Water Use | 10–15% |
| 6 | Energy Resources | 10–15% |
| 7 | Atmospheric Pollution | 7–10% |
| 8 | Aquatic/Terrestrial Pollution | 7–10% |
| 9 | Global Change | 15–20% |

**Unit 9 (Global Change)** is the heaviest single unit. Units 3–6 together dominate.

## FRQ Strategy

See also: [APES FRQ guide](/blog/ap-environmental-science-frq-guide)

1. **Design an Investigation** — hypothesis, variables, control, procedure.
2. **Analyze a Problem** — identify cause/effect, propose solution with tradeoffs.
3. **Analyze with Calculation** — show math with units; interpret in environmental context.

## Study Plan

**Weeks 1–3:** Units 3–6 (populations through energy).

**Weeks 4–5:** Units 7–9 (pollution + global change).

**Week 6:** FRQ practice from AP Central released exams.

**Week 7:** Mock + [APES score calculator](/tools/ap-score-calculator/ap-environmental-science).

[Practice AP Environmental Science](/exams/ap-environmental-science)

## Related Guides

- [APES FRQ guide](/blog/ap-environmental-science-frq-guide)
- [Fully digital vs hybrid AP exams](/blog/fully-digital-vs-hybrid-ap-exams)
- [Free official AP resources](/blog/free-official-ap-study-resources)

---

*Sources: College Board AP Environmental Science CED (apcentral.collegeboard.org). Verified July 2026.*
