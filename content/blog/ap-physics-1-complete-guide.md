---
title: AP Physics 1 Complete Study Guide (2026 Exam)
description: >-
  Master AP Physics 1 for May 2026: redesigned hybrid format, 8 units with
  weights, FRQ types, and a focused study plan with free practice.
date: '2026-07-19'
updated: '2026-07-19'
author: AP Practice Exam Online Team
category: Study Guides
focusKeyword: ap physics 1 study guide
seoTitle: AP Physics 1 Study Guide 2026 (Free)
metaDescription: >-
  Complete AP Physics 1 guide: 40 MCQs + 4 FRQs, unit weights, calculator
  policy, and free Bluebook-style practice + score calculator.
tags:
  - AP Physics 1
  - AP Science
  - Study Guide
draft: false
faq:
  - question: Is AP Physics 1 calculus-based?
    answer: >-
      No. Physics 1 is algebra-based. You need strong algebra and trigonometry,
      not calculus.
  - question: What changed in the 2025 Physics 1 redesign?
    answer: >-
      The exam now has 40 MCQs and 4 FRQs (50/50 weighting), down from the older
      50 MCQ / 5 FRQ format.
  - question: Which units are heaviest?
    answer: >-
      Units 2 (Force) and 3 (Work, Energy, Power) each account for 18–23% of the
      exam.
---
**Short answer:** The 2026 AP Physics 1: Algebra-Based exam is Wed, May 6, 2026 (12 PM Local) — 40 MCQs, 180 minutes total, hybrid digital (MCQ in Bluebook, paper FRQs).

The 2026 AP Physics 1 exam is **Wednesday, May 6, 2026 (12 PM local)**. Algebra-based mechanics covering kinematics through fluids. Pass rates improved significantly in 2025 (~66% earned 3+) after the exam redesign.

## Exam Structure (2025–26 CED)

Hybrid digital — **3 hours**. Calculator permitted on both sections.

| Section | Questions | Time | Weight |
|---|---|---|---|
| Section I (MCQ) | 40 | 80 min | 50% |
| Section II (FRQ) | 4 | 100 min | 50% |

## Unit Weights

| Unit | Topic | Weight |
|---|---|---|
| 1 | Kinematics | 10–15% |
| 2 | Force and Translational Dynamics | 18–23% |
| 3 | Work, Energy, and Power | 18–23% |
| 4 | Linear Momentum | 10–15% |
| 5 | Torque and Rotational Dynamics | 10–15% |
| 6 | Energy/Momentum of Rotating Systems | 5–8% |
| 7 | Oscillations | 5–8% |
| 8 | Fluids | 10–15% |

**Units 2 + 3 = ~36–46%** of your score. Master free-body diagrams and energy bar charts first.

## FRQ Archetypes

1. **Experimental design** — propose a procedure, identify variables, justify equipment.
2. **Qualitative/quantitative translation** — explain then calculate.
3. **Paragraph-length response** — defend a claim with physics principles.

Always **show units**, **draw diagrams**, and **cite fundamental principles** (Newton's laws, conservation laws).

## Study Plan

**Weeks 1–2:** Units 1–2 (kinematics + forces). 20 MCQ/day.

**Weeks 3–4:** Units 3–4 (energy + momentum). Practice energy bar charts.

**Weeks 5–6:** Units 5–8 (rotation, oscillations, fluids).

**Week 7:** Released FRQs + [Physics 1 score calculator](/tools/ap-score-calculator/ap-physics-1).

## Free Resources

- [AP Physics 1 CED](https://apcentral.collegeboard.org/courses/ap-physics-1-algebra-based)
- Flipping Physics / Khan Academy
- [AP Physics 1 practice tests](/exams/ap-physics-1)

[Practice AP Physics 1 now](/exams/ap-physics-1).

## Related Guides and Tools

- [AP Physics 1 score calculator](/tools/ap-score-calculator/ap-physics-1)
- [Physics 1 vs 2 vs C](/blog/ap-physics-1-vs-2-vs-c)
- [Hybrid FRQ booklets](/blog/hybrid-ap-exam-frq-booklets)

---

*Sources: College Board AP Physics 1 Course and Exam Description (apcentral.collegeboard.org). Verified July 2026.*
