...
/**
 * POST /api/upload/[id]/save-graph – Save cropped graph image to Storage and update question.image_url.
...
export async function POST(
  ...
  try {
    ...
    if (!isOwner && !isPublished) {
      return NextResponse.json(
        { error: "You can only save graphs for your own exams or published exams." },
        { status: 403 }
      );
    }
    ...
    if (!bbox || typeof bbox !== "object") {
      return NextResponse.json({ error: "Question has no bbox; graph save not applicable." }, { status: 400 });
    }
    ...
    if (uploadError) {
      ...
      return NextResponse.json(
        { error: "Could not save graph image. Please try again." },
        { status: 500 }
      );
    }
    ...
    if (updateError) {
      ...
      return NextResponse.json(
        { error: "Graph saved but failed to update question. Please refresh." },
        { status: 500 }
      );
    }
