...
export async function processMailWorkerBatch(
  jobId: string | null
  ...
  const recipients = (Array.isArray(job.recipients) ? job.recipients : []) as RecipientRow[];
...
/**
 * GET /api/internal/mail-worker
 * Vercel Cron entry (Authorization: Bearer CRON_SECRET).
 */
export async function GET(request: NextRequest) {
  if (!isAuthorizedMailWorkerRequest(request)) {
