...
/**
 * POST /api/moderator/exams/[id]/approve?examKind=frq
 */
export async function POST(
  ...
  try {
    ...
    if (updateError) {
      console.error("moderator approve:", updateError);
      return NextResponse.json({ error: "Could not approve exam." }, { status: 500 });
    }

    return NextResponse.json({ success: true, moderationStatus: "approved" });
  } catch (e) {
    console.error("moderator/exams approve:", e);
